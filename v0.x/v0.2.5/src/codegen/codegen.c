...
int codegen_generateExecutable(ASTNode* ast, const char* outPath) {
    ...
    // Invoke gcc to link the executable, linking the System library source
    char cmd[1024];
    snprintf(cmd, sizeof(cmd), "gcc -no-pie -o %s %s %s -I./include -L. -lm", outPath, "build/out.s", "lib/minolib/System/System.c");
    int rc = system(cmd);
    if (rc != 0) {
