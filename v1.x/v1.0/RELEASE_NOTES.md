...
- Attach to the GitHub Release:
  - `bin/minoc` (compiled compiler binary for the release platform)
  - `examples/*.mino` (example sources)
  - `build/out.s` (sample generated assembly for an example) — note `make clean` removes it
  - `lib/minolib/System/System.c` (runtime helpers)
  - `source.tar.gz` (source snapshot)
- In the release notes, mention platform/ABI (x86_64 Linux) and how `bin/minoc` was built.
